...
public class fixed_deposits extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            // Check if the logged-in user is the same as the user trying to make the deposit
            if (!loggedInUser.equals(name)) {
                response.sendRedirect("fixed_deposit.jsp?error=fd_creation_failed");
                return;
            }
            ...
            String checkExistingQuery = "SELECT * FROM current_account WHERE account_number = ?";
            pstmt = conn.prepareStatement(checkExistingQuery);
            ...
            // Insert fixed deposit record into the database
            String sql = "INSERT INTO fixed_deposits (account_number, name, deposit_amount, deposit_duration) VALUES (?, ?, ?, ?)";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                if (rowsAffected > 0) {
                    ...
                    response.sendRedirect("fixed_deposit.jsp?success=fd_created");
                } else {
                    response.sendRedirect("fixed_deposit.jsp?error=fd_creation_failed");
                }
            }
            ...
            Logger.getLogger(fixed_deposits.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    // Method to forward the error message to the fixed_deposit.jsp page
    private void forwardToFormWithMessage(HttpServletRequest request, HttpServletResponse response, String message) throws ServletException, IOException {
        ...
        RequestDispatcher dispatcher = request.getRequestDispatcher("fixed_deposit.jsp");
    ...
    private void generateReceipt(HttpServletResponse response, String username, String name, double amount, int duration) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            out.println("Fixed Deposit Receipt");
            out.println("---------------------");
