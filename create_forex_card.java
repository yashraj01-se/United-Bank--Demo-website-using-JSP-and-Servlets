...
public class create_forex_card extends HttpServlet {
    ...
    // Minimum income and credit score thresholds for Forex card
    private static final double MIN_INCOME = 100000.0; // Minimum income (in INR)
    private static final int MIN_CREDIT_SCORE = 1000;  // Minimum credit score
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String amountRequiredStr = request.getParameter("amountRequired");
        ...
        if (!isValid) {
            response.sendRedirect("create_forex_card.jsp?error=not");
            return;
        }
        ...
        if (isVerified) {
            // If details are verified, proceed with saving the forex card application data
            boolean isSuccess = saveForexCardApplication(fullName, email, phone, address, cardType, destination, amountRequiredStr, incomeStr, creditScoreStr);

            if (isSuccess) {
                response.sendRedirect("create_forex_card.jsp?success=ok"); // Redirect to success page
            } else {
                response.sendRedirect("create_forex_card.jsp?error=not_inserted"); // Redirect to error page
            }
        } else {
            // If verification fails, redirect to error page
            response.sendRedirect("create_forex_card.jsp?error=invalid"); // Redirect to a page showing mismatch error
        }
    }
    ...
    // Method to save the forex card application into the database
    private boolean saveForexCardApplication(String fullName, String email, String phone, String address, String cardType, String destination, String amountRequiredStr, String income, String creditScore) {
        ...
        try {
            ...
            // SQL query to insert the application data into the forex_card_application table
            String sql = "INSERT INTO forex_card(full_name, email, phone, address, card_type, destination, amount_required, income, credit_score) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            stmt = connection.prepareStatement(sql);
            ...
            stmt.setString(7, amountRequiredStr);
            stmt.setString(8, income);
            stmt.setString(9, creditScore);
