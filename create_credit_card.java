...
public class create_credit_card extends HttpServlet {
    ...
    // Minimum income and credit score thresholds
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!isValid) {
            response.sendRedirect("create_credit_card.jsp?error=not");
            return;
        }
        ...
        if (isVerified) {
            // If details are verified, proceed with saving the application data
            boolean isSuccess = saveCreditCardApplication(fullName, email, phone, address, cardType, incomeStr, creditScoreStr);

            if (isSuccess) {
                response.sendRedirect("create_credit_card.jsp?success=ok"); // Redirect to success page
            } else {
                response.sendRedirect("create_credit_card.jsp?error=not_inserted"); // Redirect to error page
            }
        } else {
            // If verification fails, redirect to error page
            response.sendRedirect("create_credit_card.jsp?error=invalid"); // Redirect to a page showing mismatch error
        }
    }
    ...
    // Method to save the credit card application into the database (optional)
    private boolean saveCreditCardApplication(String fullName, String email, String phone, String address, String cardType, String income, String creditScore) {
        ...
        try {
            ...
            // SQL query to insert the application data into the credit card applications table
            String sql = "INSERT INTO credit_card (full_name, email, phone, address, card_type, income, credit_score) VALUES (?, ?, ?, ?, ?, ?, ?)";
            stmt = connection.prepareStatement(sql);
