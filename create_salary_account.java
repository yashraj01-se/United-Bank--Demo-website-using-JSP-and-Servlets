...
public class create_salary_account extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (balanceStr != null && !balanceStr.isEmpty()) {
            try {
                ...
                if (balance < 0) {
                    response.sendRedirect("create_salary_account.jsp?error=invalid_balance");
                    return;
                }
            } catch (NumberFormatException e) {
                response.sendRedirect("create_salary_account.jsp?error=invalid_balance_format");
                return;
            }
        }
        ...
        try {
            ...
            if (name == null || email == null || phone == null || address == null || dob == null 
                ...
                response.sendRedirect("create_salary_account.jsp?error=missing_fields");
                return;
            }
            ...
            if (!rs.next()) {
                response.sendRedirect("create_salary_account.jsp?error=information_mismatched");
                return;
            }
            

            // Check if the account number already exists
            String checkAccountQuery = "SELECT account_number FROM salary_account WHERE account_number = ?";
            pstmt = conn.prepareStatement(checkAccountQuery);
            pstmt.setString(1, accountNumber);
            rs = pstmt.executeQuery();

            if (rs.next()) {
                response.sendRedirect("create_salary_account.jsp?error=account_number_exists");
                return;
            }

            // Insert into salary_account
            String insertSalaryQuery = 
                "INSERT INTO salary_account ( full_name, email, phone, address, dob, account_type, national_id, account_number) " +
                "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?)";
            pstmt = conn.prepareStatement(insertSalaryQuery);
            ...
            if (rowsInserted <= 0) {
                conn.rollback();
                response.sendRedirect("create_salary_account.jsp?error=salary_account_creation_failed");
                return;
            }
            ...
            if (rowsInserted <= 0) {
                conn.rollback();
                response.sendRedirect("create_salary_account.jsp?error=account_creation_failed");
                return;
            }

            // Commit transaction
            conn.commit();
            response.sendRedirect("create_salary_account.jsp?success=account_created");
            ...
            response.sendRedirect("create_salary_account.jsp?error=" + e.getMessage());
        } finally {
            // Clean up resources
            try {
                if (rs != null) rs.close();
