...
public class create_debit_card extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!isValid) {
            response.sendRedirect("create_debit_card.jsp?error=not");
            return;
        }
        ...
        if (isVerified) {
            ...
            if (isSuccess) {
                response.sendRedirect("create_debit_card.jsp?success=ok"); // Redirect to success page
            } else {
                response.sendRedirect("create_debit_card.jsp?error=not_inserted"); // Redirect to error page
            }
        } else {
            // If verification fails, redirect to error page
            response.sendRedirect("create_debit_card.jsp?error=invalid"); // Redirect to a page showing mismatch error
        }
    }
    ...
    private boolean saveCreditCardApplication(String fullName, String email, String phone, String address, String cardType, String income, String creditScore) {
        ...
        try {
            ...
            String sql = "INSERT INTO debit_card_(full_name, email, phone, address, card_type, income, credit_score) VALUES (?, ?, ?, ?, ?, ?, ?)";
            stmt = connection.prepareStatement(sql);
